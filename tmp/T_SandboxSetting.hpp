...
#ifndef T_SANDBOXSETTING
#define T_SANDBOXSETTING
#include "O_SandboxSetting.hpp"

namespace DMMM {

class T_SandboxSetting{
public:
    ...
    T_SandboxSetting() 
    {
        _tables.push_back("sandbox_settings");
    }
    T_SandboxSetting(const Condition& c) 
        : _constraint(c)
    {
        _tables.push_back("sandbox_settings");
    }
    ...
    struct E_id{
        E_id() 
        {  
            _field = "sandbox_settings.id";
        }
        std::string _field;
        typedef T_SandboxSetting::Condition ConditionType;
        typedef Field<I_SandboxSetting>::Base ComparerType;
    };
    ...
    struct E_prop_group{
        E_prop_group() 
        {  
            _field = "sandbox_settings.prop_group";
        }
        std::string _field;
        typedef T_SandboxSetting::Condition ConditionType;
    ...
    struct E_short_name{
        E_short_name() 
        {  
            _field = "sandbox_settings.short_name";
        }
        std::string _field;
        typedef T_SandboxSetting::Condition ConditionType;
        typedef F_String::Base ComparerType;
    ...
    struct E_default_val{
        E_default_val() 
        {  
            _field = "sandbox_settings.default_val";
        }
        std::string _field;
        typedef T_SandboxSetting::Condition ConditionType;
        typedef F_String::Base ComparerType;
    ...
    struct E_prop_type{
        E_prop_type() 
        {  
            _field = "sandbox_settings.prop_type";
        }
        std::string _field;
        typedef T_SandboxSetting::Condition ConditionType;
        typedef Field<EnumSandboxSettingPROP_TYPE>::Base ComparerType;
    };
    ...
    std::vector<O_SandboxSetting> 
    select(const Condition& c, const std::string& additional)
    {
        std::vector<O_SandboxSetting> ret;
        ...
        for(size_t i = 0; i < res.size(); ++i){
            ret[i]._f_id._base =
                UTILS::fromString<Field<I_SandboxSetting>::Base>(res[i]["id"]);
            ...
            ret[i]._f_prop_type._base =
                (Field<EnumSandboxSettingPROP_TYPE>::Base)UTILS::fromString<size_t>(res[i]["prop_type"]);
        }
        return ret;
    }
    ...
    std::vector<O_SandboxSetting> select(const Condition& c)
    {
        Condition c1 = _constraint.nil() ? c : _constraint && c;
        return select(c1, "");
    }
    ...
    std::vector<O_SandboxSetting> select()
    {
        return select(_constraint, "");
    }

    std::pair<O_SandboxSetting, bool> select(const I_SandboxSetting& id){
        return first(E_id() == id);
    }

    std::pair<O_SandboxSetting, bool> first(const Condition& c)
    {
        Condition c1 = _constraint.nil() ? c : _constraint && c;
        std::vector<O_SandboxSetting> r = select(c, "limit 1");
        if(r.size() > 0)
            return std::make_pair(r[0], true);
        else
            return std::make_pair(O_SandboxSetting(), false);
    }

    std::pair<O_SandboxSetting, bool> first(){
        return first(_constraint);
    }

    bool insertAllFields(std::vector<O_SandboxSetting>::const_iterator begin,
                         std::vector<O_SandboxSetting>::const_iterator end)
    {
	...
	for (; begin != end; ++begin){
	    const O_SandboxSetting& r = *begin;
        ...
        return DBFace::instance()->insert("sandbox_settings",
					  fields, rows);
    }

    bool insertDirtyFields(std::vector<O_SandboxSetting>::const_iterator begin,
                           std::vector<O_SandboxSetting>::const_iterator end)
    {
        if (begin == end)
            return true;
        FieldsToRows fields2Rows;
	for (std::vector<O_SandboxSetting>::const_iterator it = begin; 
             it != end; ++it)
        {
            std::vector<std::string> fields;
            std::vector<std::string> row;
        ...
        {
            ret = DBFace::instance()->insert("sandbox_settings",
